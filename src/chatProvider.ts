...
export class OpenClaudeViewProvider implements vscode.WebviewViewProvider {
  ...
  private async _saveCurrentChat() {
    let conversations = this._context.globalState.get<Conversation[]>("openclaude.history") || [];
    ...
    if (firstUserMsg) {
      if (typeof firstUserMsg.content === "string") {
        ...
        if (textBlock && textBlock.text) title = textBlock.text.substring(0, 40);
      }
    }
  ...
  private _sendHistory() {
    const conversations = this._context.globalState.get<Conversation[]>("openclaude.history") || [];
...
<head>
  ...
  <meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src ${webview.cspSource} data: https:; style-src ${webview.cspSource} 'nonce-${nonce}' https://fonts.googleapis.com; font-src https://fonts.gstatic.com; script-src 'nonce-${nonce}'; connect-src https://open-claude.com;">
